...
/**
 * A heap based priority queue where the root is the smallest element (min PQ).
 * 
...
public class MinPQ<Key_Class> {
	...
	public void insert(Key_Class x) {
		...
		if (count == pq.length - 1) {// sizen av pq �r statisk, m�ste g�ra den dynamisk f�r att klara mer data. (�kar
										// storlek vid behov)
			resize(2 * count);// st�r exakt vad metoden ska g�ra om man l�ser kommentar.
		}
		count++; // pq ska starta vid index 1.. s� ja
		pq[count] = x;
		swim(count); // sink anv�nds vid maxHeap, Vi k�r minHeap i koden. dvs minsta element i root
						// d�rav "simmar" vi nedifr�n och upp.
	...
	public void toss(Key_Class x) {
		orderOK = false; // Double the size of the queue as needed if (count ==
